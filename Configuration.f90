...
module ConfigurationClass
    ...
    contains
            ...
            type(Configuration) function init_Configuration(filename)
                ...
                if (ierr.ne.0) then
                    print *, 'Following file name provided was not found on directory:'
                    print *, filename
                    stop 1
                end if
                ...
                do k = 1, size(self%confMatrix%item)
                    ...
                    if (trim(param1)==trim(paramTag)) then 
                        ...
                        if (trim(param1).eq.(trim(paramTag) // ':' // trim(pool) // '-S')) then
                            ...
                            paramVec_S = [((param3Real-param2Real)/(max(real(MUnumber_S-1), 1e-18))*&
                                (i-1)+param2Real, i=1, MUnumber_S)]                                    
                            ...
                            read(param2,*)param2Real
                            read(param3,*)param3Real
                            paramVec_FR = [((param3Real-param2Real)/(max(real(MUnumber_FR-1), 1e-18))*&
                                (i-1)+param2Real, i=1, MUnumber_FR)]                                    
                        else if (trim(param1).eq.(trim(paramTag) // ':' // trim(pool) // '-FF')) then
                            if (MUnumber_FF>0) allocate(paramVec_FF(MUnumber_FF))
                            read(param2,*)param2Real
                            read(param3,*)param3Real
                            paramVec_FF = [((param3Real-param2Real)/(max(real(MUnumber_FF-1), 1e-18))*&
                                (i-1)+param2Real, i=1, MUnumber_FF)]                                    
