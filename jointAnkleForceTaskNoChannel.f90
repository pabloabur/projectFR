...
module jointAnkleForceTaskNoChannelClass
    use ConfigurationClass
    use MotorUnitPoolNoChannelClass
    use MusclePointerNoChannelClass
    implicit none
    private
    integer, parameter :: wp = kind( 1.0d0 )
    real(wp), parameter :: pi = 4 * atan(1.0_wp)    
    public :: jointAnkleForceTaskNoChannel

    type jointAnkleForceTaskNoChannel
        type(Configuration), pointer :: conf
        type(MusclePointerNoChannel), dimension(:), allocatable :: muscles
    ...
    end type jointAnkleForceTaskNoChannel

    interface jointAnkleForceTaskNoChannel   
        module procedure init_jointAnkleForceTask
    end interface jointAnkleForceTaskNoChannel

    contains

        type(jointAnkleForceTaskNoChannel) function   init_jointAnkleForceTask(conf, pools)
            class(Configuration), intent(in), target :: conf
            class(MotorUnitPoolNoChannel), intent(in) :: pools(:)
            ...
            do i = 1, numberOfPools
                if (pools(i)%pool == 'SOL' .or. pools(i)%pool == 'MG' .or. pools(i)%pool == 'LG' .or. pools(i)%pool == 'TA') then
                    init_jointAnkleForceTask%muscles(i) = MusclePointerNoChannel()
        ...
        subroutine atualizeAnkle(self, t, ankleAngle)
            ...
            ! '''
            class(jointAnkleForceTaskNoChannel), intent(inout) :: self
...
end module jointAnkleForceTaskNoChannelClass
