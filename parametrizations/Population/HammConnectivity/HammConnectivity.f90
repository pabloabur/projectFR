...
program HammConnectivity
    ...
    do i = 1, size(t)
        ...
        if (t(i)>10.and.t(i)<11) then
            motorUnitPools(1)%iInjected(2*(MNi)) = 50
        else
            motorUnitPools(1)%iInjected(2*(MNi)) = 0
        end if
    ...
    do i = 1, timeLength
        do j = 1, RCNumber
            write(1, '(F15.6)', advance='no') RC_mV(i, j)
        end do
        write(1, *) '' ! Line break
    end do
    close(1)
