...
module MotorUnitPoolNoChannelClass
    ! '''
    ...
    use MuscularActivationNoChannelClass
    use MuscleNoHillNoChannelClass
    use MuscleHillClass
    ...
    public :: MotorUnitPoolNoChannel

    type MotorUnitPoolNoChannel
        real(wp) :: t
        ...
        type(MotorUnitNoChannel), dimension(:), allocatable:: unit
        ...
        type(MuscularActivationNoChannel) :: Activation
        type(MuscleNoHillNoChannel) :: NoHillMuscle
    ...
    end type MotorUnitPoolNoChannel
    interface MotorUnitPoolNoChannel
        module procedure init_MotorUnitPool
    end interface MotorUnitPoolNoChannel

    contains

    type (MotorUnitPoolNoChannel) function init_MotorUnitPool(conf, pool)
        ! '''
        ! Constructor
        ! - Inputs:
        ...
        do i = 1, init_MotorUnitPool%MUnumber
            ...
            if (i <= MUnumber_S) then
                neuronKind = 'S'
                init_MotorUnitPool%unit(i) = MotorUnitNoChannel(init_MotorUnitPool%conf, init_MotorUnitPool%pool,&
        ...
        allocate(init_MotorUnitPool%matInt(init_MotorUnitPool%totalNumberOfCompartments))
        init_MotorUnitPool%v_mV(:) = 0.0
        init_MotorUnitPool%matInt(:) = 0.0
        ...
        ! #activation signal
        init_MotorUnitPool%Activation = MuscularActivationNoChannel(init_MotorUnitPool%conf,init_MotorUnitPool%pool,&
        ...
        if (trim(init_MotorUnitPool%hillModel).eq.'No') then 
            init_MotorUnitPool%NoHillMuscle = MuscleNoHillNoChannel(init_MotorUnitPool%conf, &
    ...
    function dVdt(self, t, V)
        class(MotorUnitPoolNoChannel), intent(inout) :: self
        real(wp), intent(in) :: t
        real(wp), intent(in) :: V(:)
        ...
        dVdt = (self%iIonic + self%matInt + self%iInjected &
    ...
    subroutine atualizeMotorUnitPool(self, t, dynGamma, statGamma)
        ...
        ! '''
        class(MotorUnitPoolNoChannel) , intent(inout), target:: self
        real(wp), intent(in) :: t
...
end module MotorUnitPoolNoChannelClass   