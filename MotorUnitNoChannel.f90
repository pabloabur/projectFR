...
module MotorUnitNoChannelClass
    ...
    public :: MotorUnitNoChannel

    type MotorUnitNoChannel
        ...
        real(wp) :: tSomaSpike
        integer :: index
        type(CompartmentNoChannel), dimension(:), allocatable :: Compartments ! ## Dictionary of Compartment of the Motor Unit.
    ...
    end type MotorUnitNoChannel

    interface MotorUnitNoChannel
        module procedure init_MotorUnit
    end interface MotorUnitNoChannel
    
    contains

    type(MotorUnitNoChannel) function init_MotorUnit(conf, pool, index, neuronKind, muscleThickness, skinThickness)
        ! '''
        ! Constructor
        ...
        compKind = 'dendrite'
        init_MotorUnit%Compartments(1) = CompartmentNoChannel(compKind, init_MotorUnit%conf, &
                                                     init_MotorUnit%pool, init_MotorUnit%index, &
                                                     init_MotorUnit%neuronKind)

        compKind = 'soma'
        init_MotorUnit%Compartments(2) = CompartmentNoChannel(compKind, init_MotorUnit%conf, &
                                                    init_MotorUnit%pool, init_MotorUnit%index, &
                                                    init_MotorUnit%neuronKind)                                                     
        ...
        do i = 3, NumberOfAxonNodes+2, 2
            compKind = 'internode'
            init_MotorUnit%Compartments(i) = CompartmentNoChannel(compKind, init_MotorUnit%conf, &
                                                         init_MotorUnit%pool, init_MotorUnit%index, &
                                                         init_MotorUnit%neuronKind)
            compKind = 'node'
            init_MotorUnit%Compartments(i+1) = CompartmentNoChannel(compKind, init_MotorUnit%conf, &
                                                           init_MotorUnit%pool, init_MotorUnit%index, &
                                                           init_MotorUnit%neuronKind)
        end do
    ...
    subroutine atualizeMotorUnit(self, t, v_mV)
        ...
        ! '''
        class(MotorUnitNoChannel), intent(inout) :: self 
        real(wp), intent(in) :: t
...
end module MotorUnitNoChannelClass
